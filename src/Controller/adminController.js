...
exports.accEvent = async(req,res) =>{
  try{
    ...
    await Event.updateOne({id_event: event[0].id_event},{
      $set: {
        isVerified: true
      }
    });
...
exports.deleteMahasiswa = async(req,res) =>{
  try{
    ...
    const test = await Event.updateMany(
      {},
      {$pull: {registered_people:{email:mahasiswa.email}}},
			{multi: true}
...
exports.deleteOrmawa = async(req,res) =>{
  try{
    ...
    await Mahasiswa.updateMany(
			{},
			{$pull: {historyEvent:{id_event: {$in: id_event}}}},
			{multi: true}
			);
    return res.redirect('/pengguna-ormawa');
  }
...
exports.editLandingPage = async(req,res) =>{
  try{
    const landing = await LandingPage.findOne({id:1});
    await LandingPage.updateOne({id:1},{
      $set: {
